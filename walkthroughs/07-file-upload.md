...
| File | Role |
|------|------|
| `api/src/main/java/com/securetask/entity/Attachment.java` | `id`, `task`, `originalFilename`, `storageKey`, `contentType`, `fileSize`, `uploadedAt` |
| `api/src/main/java/com/securetask/repository/AttachmentRepository.java` | `findByTask`, `findByIdAndTask` |
| `api/src/main/java/com/securetask/dto/AttachmentResponse.java` | Exposed fields — **no `storageKey`** |
| `api/src/main/java/com/securetask/service/FileTypeValidator.java` | Magic bytes check — shared by both parts |
| `api/src/main/java/com/securetask/service/FileStorageService.java` | Interface: `store`, `load`, `delete` |
| `api/src/main/java/com/securetask/service/LocalFileStorageService.java` | Local filesystem backend — used by task attachments |
| `api/src/main/java/com/securetask/service/AttachmentService.java` | Business logic, ownership checks |
| `api/src/main/java/com/securetask/controller/AttachmentController.java` | `POST/GET/GET/{id}/DELETE /api/tasks/{taskId}/attachments` |
| `frontend/js/attachments.js` | Attachment UI (upload, list, delete, download link) |
...
// The old pattern stored files under the static resources directory; the frontend
// files are now in `frontend/` and uploads are outside both.
...
Start the app (see README). Open the browser UI at http://localhost:8081, log in, and open Tasks. Click **Attachments** on any task row.
...
curl -s -F "file=@photo.png;filename=../../evil.txt" \
...
| File | Role |
|------|------|
| `api/src/main/java/com/securetask/entity/User.java` | `avatarKey` (UUID), `avatarContentType` (magic-detected) |
| `api/src/main/java/com/securetask/dto/ProfileResponse.java` | `hasAvatar`, `storageBackend` — no `avatarKey` |
| `api/src/main/java/com/securetask/service/FileTypeValidator.java` | Magic bytes check — shared with Part A |
| `api/src/main/java/com/securetask/service/S3FileStorageService.java` | S3 backend — used by profile avatar |
| `api/src/main/java/com/securetask/service/ProfileService.java` | Avatar upload, download, delete |
| `api/src/main/java/com/securetask/controller/ProfileController.java` | `GET/POST /api/profile/avatar`, `DELETE /api/profile/avatar` |
| `api/src/main/java/com/securetask/config/S3ClientConfig.java` | `S3Client` bean — endpoint override for LocalStack |
| `localstack/init/02-create-s3-bucket.sh` | Creates `securetask-uploads` bucket on LocalStack startup |
| `frontend/profile.html` | Profile page with avatar upload UI |
...
Open the browser UI at http://localhost:8081, log in, and open the Profile page. Upload a PNG image as your avatar.
...
```bash
curl -H "Authorization: Bearer $TOKEN" http://localhost:8080/api/profile/avatar
