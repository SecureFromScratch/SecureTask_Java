...
| File | Role |
|------|------|
| `entity/Task.java` | JPA entity: id, owner, title, description, status, createdAt, updatedAt |
| `entity/TaskStatus.java` | Enum: OPEN, IN_PROGRESS, DONE |
| `repository/TaskRepository.java` | `findByOwner`, `findByIdAndOwner` |
| `dto/TaskCreateRequest.java` | title `@NotBlank @Size(max=200)`, description `@Size(max=2000)` |
| `dto/TaskUpdateRequest.java` | title `@NotBlank @Size(max=200)`, description `@Size(max=2000)`, status `@NotNull` |
| `dto/TaskResponse.java` | Safe response — no internal fields |
| `service/TaskService.java` | Business logic; `@PreAuthorize` on every method |
| `controller/TaskController.java` | REST endpoints; `@Valid` on every request body |
| `controller/GlobalExceptionHandler.java` | Maps `MethodArgumentNotValidException` → 400 `{"errors":[…]}` |
| `src/main/resources/static/tasks.html` | Task management page |
| `src/main/resources/static/js/tasks.js` | Page logic — `textContent` throughout |
| `src/main/resources/static/js/api.js` | `api.tasks.{list, create, update, delete}` namespace |
...
# Send a blank title:
curl -b cookies.txt -H "X-XSRF-TOKEN: $CSRF" \
...
The server stores the payload as plain text. The JSON response contains it as a string. Open `/tasks.html` — the task title appears literally in the table cell. No alert fires. The `textContent` assignment in `tasks.js` treated the angle brackets as text, not HTML.

### Step 5 — "Break it": switch to innerHTML

Open `src/main/resources/static/js/tasks.js`. Find the line:
...
Reload `/tasks.html`. The alert fires. The `<img>` tag was parsed as HTML, and the `onerror` attribute executed as JavaScript.
...
Reload. The payload is displayed as literal text again. No alert fires.
...
Alice and Bob each create tasks. Log in as Alice and call `GET /api/tasks` — Bob's tasks do not appear. `TaskService.listOwn()` uses `findByOwner(owner)` where `owner` is resolved from the server-side session, not from any client-supplied parameter.
