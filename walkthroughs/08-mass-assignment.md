...
| File | Role |
|------|------|
| `api/src/main/java/com/securetask/entity/Task.java` | `completedAt` (server-managed), `pinned` (admin-only) |
| `api/src/main/java/com/securetask/dto/TaskCreateRequest.java` | `title`, `description` only — no `pinned`, no `completedAt` |
| `api/src/main/java/com/securetask/dto/TaskUpdateRequest.java` | `title`, `description`, `status` only |
| `api/src/main/java/com/securetask/dto/TaskPinRequest.java` | `{ pinned: boolean }` — body for the admin pin endpoint |
| `api/src/main/java/com/securetask/dto/TaskResponse.java` | Exposes `completedAt` and `pinned` in responses |
| `api/src/main/java/com/securetask/service/TaskService.java` | Manages `completedAt` on status transition; `pin()` is admin-only |
| `api/src/main/java/com/securetask/controller/TaskController.java` | `PATCH /api/tasks/{id}/pin` — requires `ADMIN` role |
...
### Step 1 — Introduce the vulnerability manually

To see the attack in action, temporarily introduce the vulnerable fields.

**In `api/src/main/java/com/securetask/dto/TaskCreateRequest.java`**, add one field:
```java
private boolean pinned = false;  // VULNERABLE — remove after this step
```

**In `api/src/main/java/com/securetask/service/TaskService.java`**, find `create()` and add one line before `return TaskResponse.from(taskRepository.save(task))`:
```java
task.setPinned(request.isPinned());  // VULNERABLE — remove after this step
```

**In `api/src/main/java/com/securetask/dto/TaskUpdateRequest.java`**, add two fields:
```java
private java.time.Instant completedAt;  // VULNERABLE — remove after this step
private boolean pinned;                 // VULNERABLE — remove after this step
```

**In `api/src/main/java/com/securetask/service/TaskService.java`**, find `update()` and add two lines before `return TaskResponse.from(taskRepository.save(task))`:
```java
task.setCompletedAt(request.getCompletedAt());  // VULNERABLE — remove after this step
task.setPinned(request.isPinned());              // VULNERABLE — remove after this step
```

Restart: `./gradlew :api:bootRun`

Get a token and run the attacks:
...
# A regular user pins their own task — no admin check:
...
Now try to set a backdated completion time — get a task ID from the list first:
...
# → "completedAt": "2015-06-15T09:00:00Z" — attacker tampers with the audit trail
```

**Revert all four changes** (remove the added lines), then restart `./gradlew :api:bootRun`.
...
- [ ] Run `./gradlew :api:test` → all tests pass
