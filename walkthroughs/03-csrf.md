...
1. Alice is logged in to `securetask.example.com`. Her browser holds a valid `JSESSIONID` cookie.
...
5. Alice's browser attaches her `JSESSIONID` cookie to the request.
6. The server sees an authenticated request and — without CSRF protection — executes it.
...
`SecurityConfig.java` configures a `CookieCsrfTokenRepository`:

```java
CookieCsrfTokenRepository csrfRepo = CookieCsrfTokenRepository.withHttpOnlyFalse();
```

- Spring Security generates a random token and stores it server-side, tied to the session.
...
- `withHttpOnlyFalse()` is deliberate — the JavaScript client must be able to read this cookie.
  The session cookie (`JSESSIONID`) remains `HttpOnly=true`, so XSS cannot steal the session.
...
`CsrfCookieFilter.java` solves this:
...
This filter runs on every request. The `XSRF-TOKEN` cookie is therefore always fresh.

### Token submission

`api.js` reads the cookie and sends it as a custom request header on every state-changing request:
...
Spring Security's `CsrfFilter` intercepts every `POST`, `PATCH`, `PUT`, and `DELETE` request. It compares the `X-XSRF-TOKEN` header (or `_csrf` form parameter) against the token stored in the session. If they do not match, the request is rejected with `403 Forbidden`.
...
| File | Purpose |
|------|---------|
| `src/main/java/com/securetask/security/SecurityConfig.java` | CSRF configuration — repository, handler, filter |
| `src/main/java/com/securetask/security/CsrfCookieFilter.java` | Forces the XSRF-TOKEN cookie to be written on every response |
| `src/main/resources/static/js/api.js` | `getCsrfToken()` + `X-XSRF-TOKEN` header on all state-changing requests |
| `src/main/resources/application.properties` | `SameSite=Strict` and `HttpOnly=true` on the session cookie |
...
The CSRF token is sent as a form parameter. Spring Security's form login processor reads it from `_csrf`.

### Step 3 — Watch the token flow through a state-changing API call

Still in the Network tab, go to the admin panel and change a user's role.
...
This header cannot be set by a cross-origin form. Only same-origin JavaScript can set custom headers.

### Step 4 — Demonstrate a rejected request (missing token)

Open a browser console on http://localhost:8080 (same origin) and run:
...
The server rejected the request because `X-XSRF-TOKEN` was absent.
...
Open a console on http://localhost:8080 and try submitting a form-encoded request without the token (as a cross-origin form would):
...
The request was rejected because it did not include the `X-XSRF-TOKEN` header or `_csrf` parameter — exactly what an HTML form submitted from `evil.example.com` would look like.
...
Open `CsrfCookieFilter.java`. Notice that `getToken()` is the only call — it forces the lazy token to resolve, which causes `CookieCsrfTokenRepository` to write the `XSRF-TOKEN` cookie into the response.
...
- [ ] `XSRF-TOKEN` cookie is present after loading any page
- [ ] `XSRF-TOKEN` cookie has `HttpOnly=false`
- [ ] `JSESSIONID` cookie has `HttpOnly=true` and `SameSite=Strict`
...
| `POST /api/register` with no token | absent | 403 Forbidden |
...
4. The browser automatically attaches the victim's `JSESSIONID` cookie to the request, because cookies are sent based on the destination domain, not the origin of the request.
5. The server receives an authenticated-looking request and — without CSRF protection — executes it.
...
In Spring Security, the only way to exempt a path from CSRF is to call `csrf().ignoringRequestMatchers(...)` explicitly. This application does not do that anywhere.
